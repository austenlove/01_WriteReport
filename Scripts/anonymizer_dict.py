...
# 2. 학교 사전
# ============================================
SCHOOL_DICT_PATH = Path("C:/Users/dhkim/OneDrive/문서/UiPath/전북군산교육청/Data/school_dict.txt")
    ...
    def add_school(self, original):
        for key, value in self.mapping.items():
            if ( value["type"] == "SCHOOL" and value["original"] == original ):
                print( f"기존 학교명 매핑 사용: " f"{original} → {key}" )
                return key
        ...
        print( f"---- 학교명 매핑: " f"{original} → {masked}" )
        return masked
...
# 4. 텍스트 비식별화
# ============================================================ 
def replace_school(text, manager, school_dictionary):
    ...
    for school in school_dictionary:
        ...
        masked = manager.add_school(school)

        text = text.replace(school, masked)

        print(f"---- 학교 탐지: " f"{school} → {masked}")

    return text


def anonymize_text(text, manager, school_dictionary):
    ...
    # 학교명
    text = replace_school(text, manager, school_dictionary)
...
def anonymize_hwpx(input_path, output_path, mapping_path):
    ...
    school_dictionary = load_school_dictionary(SCHOOL_DICT_PATH)
    ...
    for xml_file in xml_files:
        ...
        try:
            ...
            for element in tree.iter():
                ...
                replaced_text = anonymize_text(original_text, manager, school_dictionary)
...
if __name__ == "__main__":
    input_file = Path("C:/Users/dhkim/OneDrive/문서/UiPath/전북군산교육청/Data/Input/0601옥산초-금빛초_사안조사보고서조판철.hwpx")
    output_file = Path("C:/Users/dhkim/OneDrive/문서/UiPath/전북군산교육청/Data/Output/0601옥산초-금빛초_사안조사보고서조판철.hwpx")
    mapping_file = Path("C:/Users/dhkim/OneDrive/문서/UiPath/전북군산교육청/Data/Output/조0601옥산초-금빛초_사안조사보고서조판철.json")
